...
export async function registerMediaRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (request, reply) => created(reply, mediaService.create(mediaAssetPayloadSchema.parse(request.body))));

  app.post("/collect", async (request, reply) => accepted(reply, mediaService.collect(collectMediaSchema.parse(request.body))));
