...
async function persistToken(platform: string, providerScopes: string[], token: {
  ...
  return platformsRepository.updateAccount(platform, {
    workspace_id: metadata.workspace_id ? String(metadata.workspace_id) : undefined,
    status: "connected",
...
function decorateAccount(account: PlatformAccount) {
  ...
  const { access_token: _accessToken, refresh_token: _refreshToken, ...safeAccount } = account;
...
export const platformsService = {
  ...
  async getConnectUrl(platform: string, workspaceId?: string) {
    return this.connect(platform, workspaceId);
  },
  ...
  async disconnect(platform: string, workspaceId?: string) {
    ...
    await auditService.log({ action: "platform.disconnect", entity_type: "platform_account", entity_id: account.id, metadata: { platform } });
  ...
  async publish(platform: string, payload: PublishPayload, workspaceId?: string) {
    ...
    const result = await provider.publish(account, payload);
    await auditService.log({ action: "platform.publish", entity_type: "platform_account", entity_id: account.id, metadata: { platform, post_id: payload.post_id, media_asset_id: payload.media_asset_id } });
    return result;
  },
};
